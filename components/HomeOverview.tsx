...
const HomeOverview: React.FC<HomeOverviewProps> = ({
  ...
  const kpiData = useMemo(() => {
    ...
    const pendingSamples = samples.filter(
      s => s.status !== SampleStatus.FINISHED && s.status !== SampleStatus.DELETED
    );
    const totalSamples = samples.length;

    const activeHolds =
      holdCases.filter(c => !c.is_finished).length +
  ...
  const recentUpdates = [
    {
      id: 1,
      title: isAr ? 'تحديث النظام v2.9' : 'System Update v2.9',
      time: '2h ago',
      type: 'system',
      desc: 'Performance improvements and UI refresh.'
    },
    {
      id: 2,
      title: isAr ? 'تمت أرشفة 50 عينة' : 'Archived 50 samples',
      time: '5h ago',
      type: 'data',
      desc: 'Automatic archival of finished samples.'
    },
    {
      id: 3,
      ...
      time: '1d ago',
      type: 'alert',
      desc: 'Server maintenance scheduled for Friday.'
    },
  ];
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 min-h-full animate-in fade-in duration-500">
      ...
      {/* ✅ Notification Center في لوحة التحكم الرئيسية */}
      <NotificationCenter />
          ...
          <div className="h-[350px] w-full" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData.weeklyData}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#000000', fontSize: 11, fontWeight: 800 }} dy={15} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: '#000000', fontSize: 11, fontWeight: 800 }} />
                <RechartsTooltip
